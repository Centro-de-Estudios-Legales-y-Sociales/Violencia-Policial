...
self.addEventListener(
  ...
  function (e) {
    ...
    self.filterData = function () {
      ...
      //Claisifcacion
      medData.victimas = entities.filter(function (x) {
        return x.key === victimasTemplateId;
      })[0].values;
      medData.funcionarios = entities.filter(function (x) {
        return x.key === funcionariosTemplateId;
      })[0].values;
      ...
      medData.years = medData.hechos.map(function (x) {
        x.metadata.fecha = new Date(
          x.metadata.fecha_del_hecho[0].value * 1000 + hoursARGMT_3
        );
        x.metadata.ano = x.metadata.fecha.getFullYear();
      ...
      medData.hechos.map(function (x) {
        if (!x.metadata.victimas_civiles) {
          x.metadata.victimas_civiles = [];
          x.metadata.lista_victimas = [];
          x.metadata.victimas_fatales = 0;
          x.metadata.victimas_fatales_H = 0;
          x.metadata.victimas_fatales_M = 0;
          x.metadata.victimas_fatales_X = 0;
        }
        if (!x.metadata.funcionarios) {
          x.metadata.funcionarios = [];
          x.metadata.lista_funcionarios = [];
          x.metadata.victimas_fatales = 0;
          x.metadata.victimas_fatales_H = 0;
          x.metadata.victimas_fatales_M = 0;
          x.metadata.victimas_fatales_X = 0;
        ...
        // else {
        //Obtengo el id
        var victimasSharedIdEnHecho = [];
        if (x.metadata.victimas_civiles) {
          victimasSharedIdEnHecho = x.metadata.victimas_civiles.map(function (
            p
          ) {
            return p.value;
          });
        ...
        var stop = false;
        var funcionariosSharedIdEnHecho = [];
        if (x.metadata.funcionarios) {
          funcionariosSharedIdEnHecho = x.metadata.funcionarios.map(function (
            p
          ) {
            return p.value;
          });
        ...
        x.metadata.victimas_fatales = medData.victimas.filter(function (v) {
          var filter = false;

          v.title = v.title.replace("  ", " ");

          var victimaMatch = victimasSharedIdEnHecho.filter(function (l) {
            return l == v.sharedId;
          });

          filter =
            v.metadata.estado[0].label == "Muerto" && victimaMatch.length > 0;

          if (victimaMatch.length > 0 && !v.metadata.count) {
            v.metadata.count = true;
            x.metadata.lista_victimas.push(v);
            v.hecho = x;
            var gen = v.metadata.genero[0].label;
            if (gen == "Varón") {
              x.metadata.victimas_fatales_H += 1;
            } else if (gen == "Mujer") {
              x.metadata.victimas_fatales_M += 1;
            }
          }
        ...
        //Funcionario
        x.metadata.lista_funcionarios = [];
        x.metadata.funcionarios_fatales = 0;
        x.metadata.funcionarios_fatales_X = 0;
        x.metadata.funcionarios_fatales_M = 0;
        x.metadata.funcionarios_fatales_H = 0;

        x.metadata.funcionarios_fatales = medData.funcionarios.filter(function (
          v
        ) {
          v.title = v.title.replace("  ", " ");

          var functMatch = funcionariosSharedIdEnHecho.filter(function (l) {
            return l == v.sharedId;
          });
          var filter =
            v.metadata.estado_del_funcionario[0].label == "Muerto" &&
            functMatch.length > 0;

          if (filter) {
            x.metadata.lista_funcionarios.push(v);
            v.hecho = x;
            if (v.metadata.genero[0].label == "Varón") {
              x.metadata.funcionarios_fatales_H += 1;
            }
            if (v.metadata.genero[0].label == "Mujer") {
              x.metadata.funcionarios_fatales_M += 1;
            } else {
              x.metadata.funcionarios_fatales_X += 1;
            }
          }
      ...
      medData.muertes_genero = medData.victimas.filter(function (v) {
        return (
          v.metadata.estado[0].label == "Muerto" &&
          parseInt(v.metadata.edad[0].value) > 0
        );
      });

      medData.muertes_genero = d3
        .nest()
        .key(function (d) {
          return d.metadata.genero[0].label;
        })
        .entries(medData.muertes_genero);

      //edad
      medData.muertes_genero.map(function (vg) {
        vg.victimas_edad = d3
          .nest()
          .key(function (d) {
            return parseInt(d.metadata.edad[0].value);
          })
          .entries(vg.values);
        vg.victimas_edad.sort(function (a, b) {
      ...
      medData.provincias = medData.hechos.map(function (v) {
        return v.metadata.provincia[0];
      });
      medData.genero = medData.victimas.map(function (v) {
        return !v || !v.metadata ? { label: "" } : v.metadata.genero[0];
      });

      medData.institucion = d3
        .nest()
        .key(function (d) {
          return d.label;
        })
        .entries(medData.institucion);

      medData.provincias = d3
        .nest()
        .key(function (d) {
          return d.label;
        })
        .entries(medData.provincias);
      ...
      medData.provincias = medData.provincias.sort(function (a, b) {
        return a.key.localeCompare(b.key);
      });
      medData.institucion = medData.institucion.sort(function (a, b) {
        return a.key.localeCompare(b.key);
      });
      medData.years = medData.years
        .filter(function (y) {
          return y.key > 1995;
        })
        .sort(function (a, b) {
    ...
    ajax(
      ...
      function (dF) {
        ...
        var dFj = JSON.parse(dF);
        mainData = mainData.concat(dFj.rows);

        ajax(
          vData,
          {},
          function (dV) {
            var dVj = JSON.parse(dV);
            mainData = mainData.concat(dVj.rows);

            ajax(
              hData,
              {},
              function (dH) {
                var dHj = JSON.parse(dH);
                mainData = mainData.concat(dHj.rows);
                self.filterData();
              },
              "GET"
            ); // dh
