...
angular
    .module('app.core')
    .controller('CivilesMapController', function($http, $rootScope, $scope, medService) {
    	    $scope.currentH =[];
          ...
          $scope.exportCSV = function(){
            var itemsFormatted = [];
              ...
              var fileTitle = 'CELS-victimas-particulares-por-lugar'; // or 'my-unique-title'
          ...
          $scope.loadChart = function(h){
            ...
            var h = $scope.currentH = h.filter(function(v){
               var filtered = v.metadata.victimas_fatales > 0; 
           ...
           medService.hechosLugar.map(function(z){
               if (z.key != "Ciudad de Buenos Aires"){
